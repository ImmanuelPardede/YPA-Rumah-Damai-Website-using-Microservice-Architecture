                        ...
                        <tbody>
                            @foreach ($program as $index => $programItem)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    <td>
                                        <img src="{{ asset($programItem->img_program) }}" alt="Program Image"
                                            class="img-fluid"
                                            style="border-radius: initial; width: 100%; height: auto; max-width: 100%;">
                                    </td>
                                    <td>{!! $programItem->kelas !!}</td>
                                    <td>
                                        <a href="{{ route('program.show', $programItem->id) }}"
                                            class="btn btn-primary btn-sm">Detail</a>
                                        <a href="{{ route('program.edit', $programItem->id) }}"
                                            class="btn btn-info btn-sm">Edit</a>
                                        <form action="{{ route('program.destroy', $programItem->id) }}" method="POST"
